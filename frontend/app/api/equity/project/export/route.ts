...
const sanitizeAttachmentFilename = (filename: string) => {
  const sanitized = sanitizeFilenamePart(filename);
  return sanitized || "equity-artifact";
};

const normalizeProjectForExport = (value: unknown): EquityProject => {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const project = normalizeProjectForExport(body.project);
    const result = await buildEquityExport({ artifactType, project });
    const filename = sanitizeAttachmentFilename(result.filename);
    const bodyBuffer = new Uint8Array(result.buffer);
    ...
    return NextResponse.json(
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to export analyst artifact.",
      },
      { status: 400 },
    );
  }
